    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        if SSDEEP_LIBRARY_AVAILABLE:
            try:
                ...
                logger.debug(f"SSDeep hash calculated using Python library: {results['ssdeep_hash']}")
                return results
        ...
        try:
            ...
            if results['ssdeep_hash']:
                logger.debug(f"SSDeep hash calculated using system binary: {results['ssdeep_hash']}")
                return results
        except Exception as e:

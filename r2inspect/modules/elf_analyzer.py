...
from ..utils.r2_helpers import safe_cmdj, safe_cmd_list, safe_cmd_dict
...
class ELFAnalyzer:
            ...
            if debug_info and 'No debug info' not in debug_info:
                ...
                for line in dwarf_lines:
                    ...
                    if 'DW_AT_comp_dir' in line or 'compilation' in line.lower():
                        ...
                        date_match = re.search(r'(\d{4}-\d{2}-\d{2}|\w{3}\s+\w{3}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2}\s+\d{4})', line)
                        if date_match:
                            ...
                            for line in lines:
                                if line.strip():
                                    ...
                                    if len(hex_match) > 4:  # Skip note header
                                        return ''.join(hex_match[4:])  # Build ID starts after header
    ...
    def _get_program_headers(self) -> List[Dict[str, Any]]:
        ...
        try:
            # Get program headers
            ph_info = safe_cmd_list(self.r2, 'iHj')

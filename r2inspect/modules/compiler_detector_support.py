...
def apply_best_compiler(
    ...
    *,
    ...
    # When the top score is held entirely by string-only signatures, a
    # corroborated multi-category candidate beneath it is stronger evidence than
    # a single bare-word match: a .NET assembly scored Pascal/Swift 1.0 (those
    # bare words appear in its metadata) above the real DotNet 0.667. Prefer the
    # best corroborated candidate when one exists. With no corroborated candidate,
    # a large all-string-only tie (>=3) is just noise from short markers hitting
    # random strings in a big binary (a 113 MB Rust PE tied D/Pascal/Fortran/
    # Swift/OCaml/Haskell/Erlang/LLVM at 1.0) -> detect nothing; a smaller tie
    # falls through to the normal capped-confidence handling below.
    if tied and all(_signature_corroboration(compiler_signatures, name) <= 1 for name in tied):
        corroborated = {
            name: score
        ...
        elif len(tied) >= _STRING_ONLY_TIE_NOISE_THRESHOLD:
            return

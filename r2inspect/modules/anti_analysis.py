...
class AntiAnalysisDetector:
    ...
    def __init__(self, r2, config):
        ...
        self.anti_debug_apis = [
            ...
            'ZwQueryInformationProcess', 'OutputDebugStringA', 'OutputDebugStringW',
            'GetTickCount', 'QueryPerformanceCounter', 'NtSetInformationThread',
            'SetThreadHideFromDebugger', 'NtQueryObject', 'NtClose'
        ]
        
        # Anti-VM artifacts
        self.vm_artifacts = [
            'VMware', 'VirtualBox', 'vbox', 'vmtoolsd', 'vmwaretray', 'vmwareuser',
            'VBoxService', 'VBoxTray', 'xenservice', 'qemu', 'bochs', 'sandboxie',
            'wireshark', 'fiddler', 'regmon', 'procmon', 'vmx', 'vhd'
        ]
        
        # Sandbox evasion patterns
        self.sandbox_indicators = [
            'sample', 'virus', 'malware', 'sandbox', 'cuckoo', 'anubis',
            'joesandbox', 'threatanalyzer', 'gfilogger', 'cwsandbox'
        ]
    ...
    def _detect_evasion_techniques(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            jmp_count = len(safe_cmd(self.r2, '/c jmp').strip().split('\n')) if safe_cmd(self.r2, '/c jmp').strip() else 0
            call_count = len(safe_cmd(self.r2, '/c call').strip().split('\n')) if safe_cmd(self.r2, '/c call').strip() else 0
        ...
        try:
            ...
            if imports:
                suspicious_categories = {
                    'Process/Thread': ['CreateProcess', 'CreateThread', 'OpenProcess', 'TerminateProcess'],
                    'Memory': ['VirtualAlloc', 'VirtualProtect', 'HeapAlloc', 'MapViewOfFile'],
                    'File System': ['CreateFile', 'DeleteFile', 'MoveFile', 'FindFirstFile'],
                    'Registry': ['RegOpenKey', 'RegSetValue', 'RegDeleteKey', 'RegEnumKey'],

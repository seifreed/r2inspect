...
class BinDiffAnalyzer:
    ...
    def _extract_structural_features(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if sections:
                ...
                features['section_names'] = sorted([s.get('name', '') for s in sections if s.get('name')])
                features['section_sizes'] = [s.get('size', 0) for s in sections]
                features['executable_sections'] = len([s for s in sections if s.get('perm', '').find('x') != -1])
                features['writable_sections'] = len([s for s in sections if s.get('perm', '').find('w') != -1])
            
            # Get imports info
            imports = safe_cmdj(self.r2, "iij", [])
            if imports:
                features['import_count'] = len(imports)
                features['imported_dlls'] = list(set([imp.get('libname', '') for imp in imports if imp.get('libname')]))
                features['imported_functions'] = [imp.get('name', '') for imp in imports if imp.get('name')]
            
            # Get exports info
            exports = safe_cmdj(self.r2, "iEj", [])
            if exports:
                features['export_count'] = len(exports)
                features['exported_functions'] = [exp.get('name', '') for exp in exports if exp.get('name')]
    ...
    def _extract_string_features(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if strings:
                ...
                features['string_signature'] = hashlib.md5('|'.join(unique_sorted).encode()).hexdigest()
    ...
    def _extract_behavioral_features(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if strings:
                string_values = [s.get('string', '') for s in strings if s.get('string')]
                features['crypto_indicators'] = len([s for s in string_values if self._has_crypto_indicators(s)])
                features['network_indicators'] = len([s for s in string_values if self._has_network_indicators(s)])
                features['persistence_indicators'] = len([s for s in string_values if self._has_persistence_indicators(s)])
            
            if imports:
                import_names = [imp.get('name', '') for imp in imports if imp.get('name')]
                features['suspicious_apis'] = len([api for api in import_names if self._is_suspicious_api(api)])
                features['crypto_apis'] = len([api for api in import_names if self._is_crypto_api(api)])
                features['network_apis'] = len([api for api in import_names if self._is_network_api(api)])
    ...
    def _generate_comparison_signatures(self, results: Dict[str, Any]) -> Dict[str, str]:
        ...
        try:
            ...
            signatures['structural'] = hashlib.md5(struct_data.encode()).hexdigest()
            ...
            signatures['function'] = hashlib.md5(func_data.encode()).hexdigest()
            ...
            signatures['string'] = hashlib.md5(string_data.encode()).hexdigest()
            ...
            signatures['behavioral'] = hashlib.md5(behav_data.encode()).hexdigest()
    ...
    def _compare_structural(self, a: Dict[str, Any], b: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if a_sections or b_sections:
                jaccard = len(a_sections & b_sections) / len(a_sections | b_sections) if (a_sections | b_sections) else 0
                score += jaccard * 0.3
            total_weight += 0.3
            ...
            if a_imports or b_imports:
                import_jaccard = len(a_imports & b_imports) / len(a_imports | b_imports) if (a_imports | b_imports) else 0
    ...
    def _compare_functions(self, a: Dict[str, Any], b: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if a_names or b_names:
                name_jaccard = len(a_names & b_names) / len(a_names | b_names) if (a_names | b_names) else 0
    ...
    def _compare_strings(self, a: Dict[str, Any], b: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if a_apis or b_apis:
                api_jaccard = len(a_apis & b_apis) / len(a_apis | b_apis) if (a_apis | b_apis) else 0
            ...
            if a_paths or b_paths:
                path_jaccard = len(a_paths & b_paths) / len(a_paths | b_paths) if (a_paths | b_paths) else 0
    ...
    def _compare_behavioral(self, a: Dict[str, Any], b: Dict[str, Any]) -> float:
        ...
        try:
            ...
            indicators = ['crypto_indicators', 'network_indicators', 'persistence_indicators', 
                         'suspicious_apis', 'crypto_apis', 'network_apis']
    ...
    def _is_api_string(self, s: str) -> bool:
        ...
        api_patterns = ['CreateFile', 'WriteFile', 'ReadFile', 'RegOpenKey', 'GetProcAddress', 
                       'LoadLibrary', 'VirtualAlloc', 'CreateThread', 'CreateProcess']
    ...
    def _has_crypto_indicators(self, s: str) -> bool:
        ...
        crypto_terms = ['encrypt', 'decrypt', 'cipher', 'hash', 'md5', 'sha', 'aes', 'rsa', 'key', 'crypto']
    ...
    def _has_network_indicators(self, s: str) -> bool:
        ...
        network_terms = ['http', 'tcp', 'udp', 'socket', 'connect', 'download', 'upload', 'url']
    ...
    def _has_persistence_indicators(self, s: str) -> bool:
        ...
        persist_terms = ['startup', 'autorun', 'service', 'registry', 'schedule', 'task']
    ...
    def _is_suspicious_api(self, api: str) -> bool:
        ...
        suspicious_apis = ['CreateRemoteThread', 'WriteProcessMemory', 'VirtualAllocEx', 
                          'SetWindowsHookEx', 'GetKeyState', 'GetAsyncKeyState', 'CreateService']
    ...
    def _is_crypto_api(self, api: str) -> bool:
        ...
        crypto_apis = ['CryptAcquireContext', 'CryptCreateHash', 'CryptEncrypt', 'CryptDecrypt']
    ...
    def _is_network_api(self, api: str) -> bool:
        ...
        network_apis = ['WSAStartup', 'socket', 'connect', 'send', 'recv', 'InternetOpen', 

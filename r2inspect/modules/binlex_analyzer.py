...
class BinlexAnalyzer:
    ...
    def analyze(self, ngram_sizes: Optional[List[int]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Binlex analysis completed: {analyzed_count}/{len(functions)} functions analyzed")
    ...
    def _analyze_function(self, func_addr: int, func_name: str, ngram_sizes: List[int]) -> Optional[Dict[int, Dict[str, Any]]]:
        """
        ...
        try:
            ...
            for n in ngram_sizes:
                if len(tokens) < n:
                    logger.debug(f"Function {func_name} has too few tokens ({len(tokens)}) for {n}-gram analysis")
                    continue
                ...
                for op in disasm['ops']:
                    if isinstance(op, dict):
                        ...
                        if mnemonic:
                            ...
                            clean_mnemonic = clean_mnemonic.replace('&nbsp;', ' ').replace('&amp;', '&')
    ...
    def _calculate_binary_signature(self, function_signatures: Dict[str, Dict[int, Dict[str, Any]]], 
                                  ngram_sizes: List[int]) -> Dict[int, str]:
        """
    ...
    def get_function_similarity_score(self, func1_ngrams: List[str], func2_ngrams: List[str]) -> float:
        """
    ...
    @staticmethod
    def calculate_binlex_from_file(filepath: str, ngram_sizes: Optional[List[int]] = None) -> Optional[Dict[str, Any]]:
        """

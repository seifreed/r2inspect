...
class RichHeaderAnalyzer:
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Rich Header extracted successfully: XOR key=0x{rich_data.get('xor_key', 0):08x}")
        ...
        try:
            ...
            if hasattr(pe.RICH_HEADER, 'values') and pe.RICH_HEADER.values:
                for entry in pe.RICH_HEADER.values:
                    if hasattr(entry, 'product_id') and hasattr(entry, 'build_version') and hasattr(entry, 'count'):
            ...
            return {
                ...
                'clear_data': pe.RICH_HEADER.clear_data.hex() if hasattr(pe.RICH_HEADER, 'clear_data') else None,
                'method': 'pefile',
                'clear_data_bytes': pe.RICH_HEADER.clear_data if hasattr(pe.RICH_HEADER, 'clear_data') else None
            }
        ...
        try:
            ...
            for rich_result in rich_results:
                for dans_result in dans_results:
                    ...
                    if rich_data:
                        logger.debug(f"Successfully extracted Rich Header at DanS:{dans_offset}, Rich:{rich_offset}")
                        return rich_data
                ...
                for start_pos in range(max(0, rich_pos - 512), rich_pos, 4):
                    if start_pos + 8 <= len(dos_stub):
                        ...
                        if len(test_data) >= 8 and len(test_data) % 8 == 0:
                            ...
                            dans_pos = start_pos
                            logger.debug(f"Estimated Rich Header start at 0x{dos_stub_start + dans_pos:x}")
                            break
    ...
    def _calculate_rich_checksum(self, data: bytes, pe_offset: int, entries: List[Dict[str, Any]]) -> int:
        """
                ...
                for dans_offset in dans_offsets:
                    for rich_offset in rich_offsets:
                        if dans_offset < rich_offset and (rich_offset - dans_offset) <= 512:
                            rich_data = self._try_extract_rich_at_offsets(dans_offset, rich_offset)
                            if rich_data:
                                logger.debug(f"Found valid Rich Header with signature pair {rich_sig}/{dans_sig}")
                                return rich_data
                    ...
                    if i + 8 <= len(data):
                        ...
                        for j in range(max(0, i-512), i):
                            if data[j:j+4] == b'DanS':
                                ...
                                if rich_data:
                                    logger.debug(f"Pattern-based search found Rich Header at DanS:{j}, Rich:{i}")
                                    return rich_data
    ...
    def _try_extract_rich_at_offsets(self, dans_offset: int, rich_offset: int) -> Optional[Dict[str, Any]]:
        """
        ...
        """
        try:
            # Calculate size of Rich Header data
            rich_size = rich_offset - dans_offset
            
            if rich_size <= 8 or rich_size > 512:  # Sanity check
                return None
            
            # Extract XOR key (4 bytes after Rich signature)
            ...
            # Skip if XOR key is 0 (likely invalid)
            if xor_key == 0:
                return None
            ...
            # Decode Rich Header
            decoded_entries = self._decode_rich_header(bytes(encoded_data), xor_key)
            
            # Validate that we got reasonable entries
            if not decoded_entries or len(decoded_entries) == 0:
                return None
            
            # Additional validation: check if entries look reasonable
            valid_entries = 0
            for entry in decoded_entries:
                prodid = entry.get('prodid', 0)
                count = entry.get('count', 0)
                if 0 < count < 10000 and 0 <= prodid < 0x10000:  # Reasonable ranges
                    valid_entries += 1
            ...
            logger.debug(f"Extracted Rich Header with {len(decoded_entries)} entries, XOR key: 0x{xor_key:08x}")
            ...
            logger.debug(f"Failed to extract Rich Header at offsets {dans_offset}-{rich_offset}: {e}")
            return None
    ...
    def _calculate_richpe_hash(self, rich_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            if 'clear_data_bytes' in rich_data and rich_data['clear_data_bytes']:
                ...
                logger.debug(f"RichPE hash calculated from pefile clear_data_bytes ({len(clear_bytes)} bytes)")
                return richpe_hash
            ...
            logger.debug(f"RichPE hash calculated from reconstructed entries ({len(clear_bytes)} clear bytes)")
            return richpe_hash
    ...
    def _debug_file_structure(self):
        ...
        try:
            ...
            # Get file size
            self.r2.cmd('s 0')
            file_size = self.r2.cmdj('ij').get('core', {}).get('size', 0)
                    ...
                    if len(data) >= 0x3C + 4:
                        ...
                        if pe_offset > 64:  # Reasonable space for Rich Header
                            ...
                            if rich_pos != -1 or dans_pos != -1:
                                start = max(0, min(rich_pos, dans_pos) - 16) if dans_pos != -1 and rich_pos != -1 else (rich_pos - 16 if rich_pos != -1 else dans_pos - 16)
                                end = min(len(stub_data), max(rich_pos, dans_pos) + 32) if dans_pos != -1 and rich_pos != -1 else (rich_pos + 32 if rich_pos != -1 else dans_pos + 32)
                    ...
                    for dans_pos in dans_positions:
                        for rich_pos in rich_positions:
                            if dans_pos < rich_pos and (rich_pos - dans_pos) < 512:
                                logger.debug(f"Attempting manual extraction at DanS:{dans_pos}, Rich:{rich_pos}")

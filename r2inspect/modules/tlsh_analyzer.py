...
class TLSHAnalyzer:
    ...
    def analyze(self) -> Dict[str, Any]:
        ...
        if not self.tlsh_available:
            return {
                'available': False,
                'error': 'TLSH library not installed'
            }
        ...
        try:
            ...
            result['stats']['sections_with_tlsh'] = sum(1 for v in result['section_tlsh'].values() if v)
            ...
            result['stats']['functions_with_tlsh'] = sum(1 for v in result['function_tlsh'].values() if v)

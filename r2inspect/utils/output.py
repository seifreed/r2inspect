...
class OutputFormatter:
        ...
        try:
            ...
            if csv_data:
                fieldnames = [
                    ...
                    'anti_sandbox', 'yara_matches', 'compiler', 'compiler_version', 'compiler_confidence', 
                    'num_functions', 'num_unique_machoc', 'num_duplicate_functions', 'num_imports', 
                    'num_exports', 'num_sections'
                ]
    ...
    def _extract_csv_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            csv_row['ssdeep_available'] = ssdeep_info.get('available', False)
            ...
            csv_row['tlsh_available'] = tlsh_info.get('available', False)
            ...
            csv_row['telfhash_available'] = telfhash_info.get('available', False)
            ...
            csv_row['rich_header_available'] = rich_header_info.get('available', False)
            csv_row['rich_header_xor_key'] = hex(rich_header_info.get('xor_key', 0)) if rich_header_info.get('xor_key') else ''
            csv_row['rich_header_checksum'] = hex(rich_header_info.get('checksum', 0)) if rich_header_info.get('checksum') else ''
            ...
            if machoc_hashes:
                ...
                num_duplicate_functions = sum(count - 1 for count in hash_counts.values() if count > 1)
        ...
        try:
            for key, value in data.items():
                ...
                if isinstance(value, dict):
                    ...
                    nested_rows = self._flatten_results(value, current_prefix)
                    rows.extend(nested_rows)
                elif isinstance(value, list):
                    # Handle lists
                    for i, item in enumerate(value):
                        if isinstance(item, dict):
                            nested_rows = self._flatten_results(item, f"{current_prefix}[{i}]")
                            rows.extend(nested_rows)
                        else:
                            rows.append({
                                'field': f"{current_prefix}[{i}]",
                                'value': str(item)
                            })
                else:
                    # Simple key-value pair
                    rows.append({
                        'field': current_prefix,
                        'value': str(value)
                    })
            ...
            rows.append({
                'field': 'error',
                'value': f'Flattening failed: {str(e)}'
            })
            ...
            if 'indicators' in self.results:
                ...
                if indicators:
                    ...
                    for indicator in indicators[:5]:  # Show first 5
                        summary_lines.append(f"  - {indicator.get('type', 'Unknown')}: {indicator.get('description', 'N/A')}")
                    if len(indicators) > 5:

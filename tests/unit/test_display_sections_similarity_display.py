...
def test_display_binlex_available():
    ...
    results = {
        "binlex": {
            ...
            "top_ngrams": {3: [("ngram1", 10), ("ngram2", 8)], 4: [("ngram3", 15)]},
        }
    }
...
def test_display_binlex_not_available():
    ...
    results = {"binlex": {"available": False, "error": "Binlex library not found"}}
    _display_binlex(results)
...
def test_add_binlex_basic_stats():
    ...
    binlex_info = {"total_functions": 100, "analyzed_functions": 95, "ngram_sizes": [3, 4, 5]}
...
def test_add_binlex_binary_signatures():
    ...
    binary_signature = {3: "a" * 100, 4: "b" * 100}
...
def test_add_binlex_top_ngrams_with_html():
    ...
    top_ngrams = {3: [("ngram&nbsp;test", 10), ("ngram&amp;test", 8)]}
...
def test_display_binbloom_available():
    ...
    results = {
        "binbloom": {
            ...
            "bloom_stats": {"average_fill_rate": 0.5, "total_filters": 10},
        }
    }
...
def test_display_binbloom_with_error():
    ...
    results = {"binbloom": {"available": False, "error": "Analysis failed"}}
    _display_binbloom(results)
...
def test_add_binbloom_similar_groups_many():
    ...
    similar_functions = [
        {"count": i, "signature": f"sig{i}", "functions": [f"f{i}"]} for i in range(5)
    ]
...
def test_add_binbloom_group_no_functions():
    ...
    group = {"count": 5, "signature": "sig1", "functions": []}
    _add_binbloom_group(table, 1, group)
    assert table.row_count == 2
...
def test_add_binbloom_bloom_stats():
    ...
    binbloom_info = {"bloom_stats": {"average_fill_rate": 0.5, "total_filters": 10}}
...
def test_display_simhash_with_error():
    ...
    results = {"simhash": {"available": False, "error": "SimHash analysis failed"}}
    _display_simhash(results)
...
def test_display_bindiff_ready():
    ...
    results = {
        "bindiff": {
            ...
            "signatures": {"imphash": "abc123", "richpe": "def456"},
        }
    }
...
def test_display_bindiff_not_ready():
    ...
    results = {"bindiff": {"comparison_ready": False, "error": "BinDiff analysis failed"}}
    _display_bindiff(results)
...
def test_display_machoc_functions_no_hashes():
    ...
    results = {"functions": {"total_functions": 100, "machoc_hashes": {}}}
    _display_machoc_functions(results)

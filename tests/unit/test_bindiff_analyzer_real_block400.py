...
def test_bindiff_error_handling(tmp_path):
    ...
    # Newer analyzer behavior degrades gracefully and can still return
    # comparison-ready output even when metadata lookups fail.
    ...
    adapter = MockAdapter(
        {
            ...
            "cfg_0x1000": {"blocks": [1, 2, 3], "edges": [[0, 1], [1, 2]]},
        }
    )
...
def test_bindiff_signatures(tmp_path):
    ...
    adapter = MockAdapter(
        {
            "file_info": {"core": {"format": "PE32"}, "bin": {"arch": "x86", "bits": 32}},
            "sections": [{"name": ".text"}],

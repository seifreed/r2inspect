...
def test_find_similar_sections_skips_malformed_section_hashes(tmp_path):
    """Malformed section_tlsh payload should degrade to an empty result."""
    if not TLSH_AVAILABLE:
        pytest.skip("TLSH not available")
    f = tmp_path / "binary.bin"
    f.write_bytes(bytes(range(256)) * 10)
    ...
    class MalformedAnalyzer(TLSHAnalyzer):
        def analyze_sections(self):
            return {"available": True, "section_tlsh": [None]}

...
def test_create_json_batch_summary_with_failures(tmp_path):
    ...
    all_results = {"test.exe": {"file_info": {"name": "test.exe"}}}
    failed_files = [("failed1.exe", "Error message 1"), ("failed2.exe", "Error message 2")]
    timestamp = "20240101_120000"
...
def test_show_summary_table_small(capsys):
    ...
    all_results = {
        f"test{i}.exe": {"file_info": {"name": f"test{i}.exe", "file_type": "PE32"}}
        for i in range(5)
    }
...
def test_collect_yara_matches_list():
    ...
    result = {"yara_matches": [{"rule": "Packer_UPX"}, {"rule": "Malware_Trojan"}]}
...
def test_collect_yara_matches_objects():
    ...
    result = {"yara_matches": [YaraMatch("Rule1"), YaraMatch("Rule2")]}
    matches = _collect_yara_matches(result)
...
def test_build_small_row_basic():
    ...
    result = {
        "file_info": {"name": "test.exe", "file_type": "PE32 executable"},
...
def test_build_summary_table_large():
    ...
    all_results = {"test.exe": {"file_info": {"md5": "abc123", "file_type": "PE32"}}}
...
def test_find_files_to_process_auto_detect(tmp_path):
    ...
    # Use the real function - empty directory yields empty list

...
class FakeAdapter:
    """Adapter returning controlled responses."""

    def __init__(self, cmdj_results=None, cmd_results=None):
        self._cmdj_results = cmdj_results or {}
        self._cmd_results = cmd_results or {}

    def cmdj(self, cmd, default=None):
        return self._cmdj_results.get(cmd, default)

    def cmd(self, command):
        return self._cmd_results.get(command, "")
...
def _make_config(deep_analysis=False):
    config = Config()
    if deep_analysis:
        config.set("analysis", "deep_analysis", True)
    return config
...
def test_should_run_full_analysis_with_config_true():
    ...
    adapter = FakeAdapter()
    config = _make_config(deep_analysis=True)
    analyzer = FunctionAnalyzer(adapter, config=config)
...
def test_get_functions_empty_adapter():
    """Test _get_functions with empty adapter (no functions)."""
    adapter = FakeAdapter()
    analyzer = FunctionAnalyzer(adapter)
...
def test_process_single_function_hash_no_mnemonics():
    """Test _process_single_function_hash with address but empty adapter."""
    adapter = FakeAdapter()
    analyzer = FunctionAnalyzer(adapter)

    func = {"name": "test_func", "addr": 0x1000, "size": 100}
    result = analyzer._process_single_function_hash(func, 0, 1)
    # With empty adapter, no mnemonics are extracted, so result is None.
    assert result is None


def test_extract_mnemonics_from_ops():
    """Test _extract_mnemonics_from_ops with real ops data."""
    adapter = FakeAdapter()
    analyzer = FunctionAnalyzer(adapter)
...
def test_try_pdfj_extraction_empty_adapter():
    """Test _try_pdfj_extraction with empty adapter."""
    adapter = FakeAdapter()
    analyzer = FunctionAnalyzer(adapter)
...
def test_try_pdj_extraction_empty_adapter():
    """Test _try_pdj_extraction with empty adapter."""
    adapter = FakeAdapter()
    analyzer = FunctionAnalyzer(adapter)
...
def test_try_basic_pdj_extraction_empty_adapter():
    """Test _try_basic_pdj_extraction with empty adapter."""
    adapter = FakeAdapter()
    analyzer = FunctionAnalyzer(adapter)
...
def test_try_pi_extraction_empty():
    """Test _try_pi_extraction with empty adapter."""
    adapter = FakeAdapter()
    analyzer = FunctionAnalyzer(adapter)
...
def test_get_function_similarity():
    ...
    hashes = {"func1": "hash_a", "func2": "hash_a", "func3": "hash_b"}

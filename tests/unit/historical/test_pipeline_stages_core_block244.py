"""Historical regression tests for core pipeline stage composition and builder wiring."""
...
def test_pipeline_parallel_timeout_and_skip():
    ...
    # Unsatisfied stages are now recorded as errors (not silently dropped)
    assert results3["b"]["success"] is False
    assert "unsatisfied" in results3["b"]["error"].lower()

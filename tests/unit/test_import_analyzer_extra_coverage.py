...
def test_analyze_import_skips_non_list_api_category_buckets():
    analyzer = _make_analyzer()
    analyzer.api_categories = {"File I/O": "CreateFile"}  # type: ignore[assignment]
    imp = {"name": "CreateFileA", "plt": 0x1000, "libname": "kernel32.dll"}
    result = analyzer._analyze_import(imp)
    assert result["category"] == "Unknown"
...
def test_analyze_api_usage_keeps_error_path_safe_for_unsized_input():
    class UnsizedImports:
        def __bool__(self) -> bool:
            return True
...
def test_build_import_statistics_all_invalid_entries_returns_empty_stats():
    stats = build_import_statistics(["bad", None, 123])  # type: ignore[list-item]
...
def test_populate_import_statistics_rejects_string_suspicious_dlls():
    result = {
        "api_analysis": {"risk_score": 10},
        "obfuscation": {"score": 10},
        "anomalies": {"count": 1},
        "dll_analysis": {"suspicious_dlls": "psapi.dll"},
        "statistics": {},
    }
    ...
    assert result["statistics"]["total_risk_score"] == 9.0

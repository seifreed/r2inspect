...
class _RichHeaderSearchableHost(RichHeaderSearchMixin):
...
def test_pattern_based_search_tries_later_dans_after_earlier_false_positive():
    ...
    searcher = _RichHeaderSearchableHost(adapter=SyntheticAdapter(bytes(data)))
...
def test_read_manual_search_bytes_no_adapter():
    searcher = _RichHeaderSearchableHost(adapter=None)
    result = searcher._read_manual_search_bytes()
    assert result is None


def test_read_manual_search_bytes_no_read_bytes_attr():
    searcher = _RichHeaderSearchableHost(adapter=NoReadBytesAdapter())
    result = searcher._read_manual_search_bytes()
    assert result is None


def test_read_manual_search_bytes_empty_data():
    searcher = _RichHeaderSearchableHost(adapter=NullReturningAdapter())
    result = searcher._read_manual_search_bytes()
    assert result is None
...
def test_manual_rich_search_with_valid_header():
    ...
    searcher = _RichHeaderSearchableHost(adapter=SyntheticAdapter(padded))
    result = searcher._manual_rich_search()
    assert result is not None
    assert "xor_key" in result
...
def test_manual_rich_search_exception_returns_none():
    ...
    searcher = _RichHeaderSearchableHost(adapter=ExplodingAdapter())
    result = searcher._manual_rich_search()
    assert result is None
...
def test_pattern_based_rich_search_exception_returns_none():
    ...
    class BrokenRichSearch(_RichHeaderSearchableHost):
...
def test_try_extract_rich_at_offsets_no_encoded_data():
    ...
    searcher = _RichHeaderSearchableHost(adapter=XorKeyOnlyAdapter())
...
def test_try_extract_rich_at_offsets_exception_returns_none():
    ...
    searcher = _RichHeaderSearchableHost(adapter=ThrowingAdapter())

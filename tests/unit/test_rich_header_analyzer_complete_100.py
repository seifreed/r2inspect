...
def test_analyze_pefile_success():
    ...
    with (
        ...
        patch(
            "r2inspect.modules.rich_header_analyzer.parse_compiler_entries",
            return_value=[{"tool": "MSVC"}],
        ),
        patch(
            "r2inspect.modules.rich_header_analyzer.calculate_richpe_hash", return_value="HASH123"
        ),
    ):
        result = analyzer.analyze()
...
def test_check_magic_bytes_success():
    ...
    with patch(
        "r2inspect.modules.rich_header_analyzer.default_file_system.read_bytes", return_value=b"MZ"
    ):
...
def test_check_magic_bytes_failure():
    ...
    with patch(
        "r2inspect.modules.rich_header_analyzer.default_file_system.read_bytes", return_value=b"XX"
    ):
        result = analyzer._check_magic_bytes()
...
def test_get_dos_stub():
    ...
    assert len(result) == len(data) - 0x40

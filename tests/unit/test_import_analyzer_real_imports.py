...
def test_import_analyzer_initialization_with_config():
    """Test ImportAnalyzer initialization with real config."""
    adapter = MinimalAdapter()
    config = Config()
...
def test_analyze_import_accepts_library_key() -> None:
    adapter = MinimalAdapter()
    analyzer = ImportAnalyzer(adapter)

    result = analyzer._analyze_import({"name": "CreateFileA", "library": "kernel32.dll"})

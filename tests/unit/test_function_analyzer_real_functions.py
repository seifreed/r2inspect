...
class SimpleConfig:
    """Simple config object for testing _should_run_full_analysis."""

    def __init__(self, deep_analysis=False):
        self.typed_config = SimpleTypedConfig(deep_analysis)


class SimpleTypedConfig:
    def __init__(self, deep_analysis):
        self.analysis = SimpleAnalysisConfig(deep_analysis)
...
class BrokenConfig:
    """Config that has typed_config set to None to trigger exception path."""
...
def test_function_analyzer_initialization_with_config():
    ...
    adapter = MinimalAdapter()
    config = SimpleConfig(deep_analysis=True)

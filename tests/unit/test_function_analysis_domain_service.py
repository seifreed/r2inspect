...
def test_classify_function_type_uses_name_and_size_heuristics() -> None:
    ...
    assert classify_function_type("msvcrt_malloc", {}) == "library"
    assert classify_function_type("ntdll_syscall", {}) == "library"
    assert classify_function_type("user32_CreateWindow", {}) == "library"
    assert classify_function_type("j_printf", {}) == "thunk"
    assert classify_function_type("thunk_CreateFile", {}) == "thunk"
    ...
    assert classify_function_type("fcn.00401000", {"size": 50}) == "user"
    assert classify_function_type("func_helper", {"size": 80}) == "user"
